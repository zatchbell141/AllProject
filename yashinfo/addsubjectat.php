...
<div class="wrapper row3">
  ...
  include 'Conn.php';
    ...
    <tr>
      <td>Seat NO:</td>
       <td><input type="text" name="txtname" value="<?php echo $seat?>"/></td>
    </tr>
    <tr>
      <td>PRN:</td>
       <td><input type="text" name="txtname" value="<?php echo $row['prn']?>"/></td>
    </tr>
    ...
    <?php
      ...
      {
    ?>
<form  method="POST" action="addbacklog.php">
      <tr>
        <th colspan='2' align="center">Record #<?php echo $i;?></th>
      </tr>
      ...
      <tr>
        ...
        <td><input type="hidden" name="prn[]" value="<?php echo $row['prn']?>" /></td>
      </tr>
      <tr>
      
        <td><input type="hidden" name="seat[]" value="<?php echo $seat ?>" /></td>
      </tr>
      <tr>
        <td>Subject Code:</td>
        <td><input type="text" name="code[]" id="code" list="browsers" onChange="textdisplay();"/>
           <datalist id="browsers">
                      <?php
                                include 'Conn.php';
                                 $query1="select code from subject";
                                  $gradenameresult=mysqli_query($con,$query1) or die(mysqli_error($con));
                                 foreach($gradenameresult as $grd)
                                 {
                                 ?>
                                 <option value="<?php echo $grd['code'];?>"><?php echo $grd['code']?></option>
                                 <?php
                                 }
                                 ?>
                    </datalist>
      ...
      <tr>
        <td>Subject Name:</td>
        <td><input type="text" name="subj[]" id="subname" /></td>
      </tr>
      <tr>
        <td>Mode:</td>
        <td><select name="mode[]">
    ...
    <?php } ?>
      <tr>
        <td colspan='2'><button class="button button2" id="submit">Insert</button></td>
      </tr>
      <tr>
...
function textdisplay()
{
  ...
  xmlhttp.open("GET","ajx.php?code="+document.getElementById("code").value,false);
  xmlhttp.send(null);
  //alert(xmlhttp.responseText);
  document.getElementById("subname").value=xmlhttp.responseText;
}
